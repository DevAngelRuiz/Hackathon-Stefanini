...
public class AlunoDTOService {
	...
	public Aluno mapAluno(AlunoDTO aluno) throws TurmaNotFoundException {
		...
		DadosPessoais dadosPessoais = new DadosPessoais(null, aluno.getCpf(), aluno.getEmail());
		dadosPessoaisService.save(dadosPessoais);

		Aluno newAluno = new Aluno(null, aluno.getNome(), aluno.getMatricula(),
				dadosPessoaisService.save(dadosPessoais), turmaService.findById(aluno.getIdTurma()));
		return newAluno;
	}
}
